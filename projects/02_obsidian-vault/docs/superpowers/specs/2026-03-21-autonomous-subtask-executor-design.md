...
- **brainstorming**: タスクファイル全体を対象に行う（全サブタスクの全体像を把握）
- **計画作成**: auto-writing-plans がサブタスク（チェックボックス）ごとのステップに分解
- **実装**: サブタスクごとに逐次実行（1つ完了 → 進捗更新 → 次のサブタスク）
- **検証**: サブタスクごとに検証を実行
...
9. サブタスクごとのループ開始:
    a. auto-executing-plans で当該サブタスクの実装を実行
    b. auto-verification で検証（テスト実行、コマンド出力の証拠収集）
       - **検証成功**: サブタスクのチェックボックスを `[x]` に更新、進捗を更新（01-tasks-update 経由）
       - **検証失敗**: Discord 通知でエスカレーション、タスク `status` を `todo` に戻す、ブランチは保持（デバッグ用）、終了
    c. 次のサブタスクへ（全サブタスク完了まで繰り返し）
10. 全サブタスク完了後、タスクファイルの `status` を `done` に更新（01-tasks-update 経由）
11. `discord-notify.sh` で完了通知（成功、変更サマリ、ブランチ名）
12. エラー・不明点発生時（任意のステップで）: 停止 + タスク `status` を `todo` に戻す + Discord 通知でエスカレーション（どのステップで失敗したか、エラー内容を含む）
